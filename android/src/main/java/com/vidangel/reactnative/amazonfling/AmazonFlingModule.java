...
public class AmazonFlingModule extends ReactContextBaseJavaModule implements LifecycleEventListener {
  ...
  @ReactMethod
  private void doSeekRelative(final String targetUuid, final String position) {
    RemoteMediaPlayer target = getRemoteMediaPlayerFromUUID(targetUuid);
    if (target != null) {
      Log.i(TAG, "try doSeekRelative...");
      long positionLong = Long.parseLong(position);
      target.seek(CustomMediaPlayer.PlayerSeekMode.Relative, positionLong).getAsync(new ErrorResultHandler("doStop", "Error Stopping"));
    }
  }
